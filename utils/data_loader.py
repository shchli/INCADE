...
class ConvECorpus(Corpus):
    ...
    def get_validation_pred(self, model, split='test'):
        ...
        for batch_num in range(max_batch_num):
            ...
            pred, attention, _, _, _, _, _, _ = model.forward(head_batch_indices)
            ...
            pred = (pred[0] + pred[1] + pred[2] + pred[3] + pred[4]) / 5.0
            ...
            pred, attention, _, _, _, _, _, _ = model.forward(tail_batch_indices)
            for i in range(pred[0].shape[0]):
